...
{
    if (size_t pos_extension = path.find_last_of(".") == std::string::npos ||
    ...
    while (std::getline(in, line) && line.size() > 2)
    {
        ...
        else if (start == "f ")
        {
            ...
            ss >> index_vertex;
            triangle.x = vertices[index_vertex];
            ...
            ss2 >> index_vertex;
            triangle.y = vertices[index_vertex];

            // Third vertex
            std::string str3(ss2.str());
            pos = str3.find(" ");
            std::stringstream ss3(str3.substr(pos));
            ss3 >> index_vertex;
            triangle.z = vertices[index_vertex];
