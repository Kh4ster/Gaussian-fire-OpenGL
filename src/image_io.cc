...
{
    uint8_t idl_length; // nombre de bits du champs d'identification de l'image
                        // commençant au bit 12h
    uint8_t color_map_type; // 01h indique si le fichier TGA contient une
                            // palette  (contient 1 si c'est le cas , 0 sinon)
    uint8_t image_type; // 02h contient le code du type de l'image contenue dans
                        // le fichier TGA
    uint16_t cmap_start; // 03h defini la position de la premiére entrée de la
                         // colormap
    ...
    uint8_t
        image_descriptor_alpha_channel_bits : 4; // 11h  contient 8 bits servant
                                                 // a décrire l'image
...
/**
 * Create a default tga header for 24 bits image without colormap, with (0,0) as
 * origin
 *
 */
tga_header new_tga_header(int width, int height)
{
    ...
    header.color_map_type =
        0; // 01h indique si le fichier TGA contient une palette  (contient 1 si
           // c'est le cas , 0 sinon)
    header.image_type = 2; // 02h contient le code du type de l'image contenue
                           // dans le fichier TGA
    header.cmap_start =
